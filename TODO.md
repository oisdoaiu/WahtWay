...
| V0.16 | 体验打磨: 错误提示优化 + Tool进度反馈 + 文件拖拽上传 + Skill编辑器 |
| V0.17 | Skill 精准化: whenToUse + allowed-tools + input_examples |
| V0.18 | Agent 认知: 文件总结 + Todo 系统 + 文件记忆 + 工作区 |
| V0.19 | 视觉与效率: 深色主题 + 命令面板 + 对话摘要 + 端口自动切换 |
...
- **A. 生态扩展**（V0.20）：多格式文件 + Skill 社区 + 自动更新
- **B. 权限模型增强**：只读命令放行 + 命令规则持久化 + 危险命令硬拦截
