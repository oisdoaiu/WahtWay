...
| # | 内容 | 说明 |
|---|------|------|
| 9 | **错误提示优化** | API 超时/断网时给出友好 toast 提示而非白屏 |
| 10 | **Tool 进度反馈** | Tool 执行耗时 >1s 时展示进度或等待状态 |
| 7 | **文件拖拽上传** | 聊天框拖入文件/文件夹，自动识别路径填入输入框 |
| 8 | **Skill 编辑器增强** | Skill 库页面直接编辑已有 Skill 的 systemPrompt/描述 |

### V0.17 — Skill 精准化（3 项，均小）
| # | 内容 | 说明 |
|---|------|------|
| 24 | **whenToUse 字段** | Skill JSON 加触发场景+反例，匹配 prompt 传给 LLM |
| 25 | **allowed-tools 白名单** | Skill 声明可调 Tool，无关 Tool 对该 Skill 不可见 |
| 31 | **input_examples** | 9 个 Tool 各加 2-3 个输入示例，降低 LLM 误用率 |

### V0.18 — Agent 认知（3 项，中）
| # | 内容 | 说明 |
|---|------|------|
| 37 | **Todo/Planning 系统** | 虚拟 Todo Tool，Agent 接复杂任务自动拆解→展示计划→逐步勾掉 |
| 38 | **文件系统记忆** | Agent 能写 `.md` 笔记到约定目录，下次读自己的笔记而非重新扫描 |
| 4 | **文件内容总结 Tool** | 新增 summarize-file Tool，读文件后调 LLM 做摘要/翻译/格式化 |

### V0.19 — 视觉与效率（3 项）
| # | 内容 | 说明 |
|---|------|------|
| 14 | **自定义主题** | 浅色/深色切换 + 自定义强调色 |
| 15 | **快捷指令** | Ctrl+K 唤起命令行模式，输入 Skill 名直达 |
| 19 | **对话摘要** | 对话列表展示 AI 自动生成的一句话摘要 |

### V0.20 — 生态扩展（3 项）
| # | 内容 | 说明 |
|---|------|------|
| 11 | **多格式文件支持** | PDF、PPTX、Word 文件解析 Tool |
| 12 | **Skill 社区** | Skill Hub 评分/评论/Fork/排行榜 |
| 17 | **自动更新** | 客户端检测新版本 → 提示下载升级 |
